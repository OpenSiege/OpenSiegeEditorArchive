...
namespace ehb
{
    ...
    void locate_install_dir(WritableConfig& config, const fs::path& steamApps)
    {
        ...
        for (const auto& path : eachSteamApps)
        {
            ...
            if (fs::is_directory(installDirDS1))
            {
                config.setString("ds-install-path", installDirDS1.string());
            }

            const fs::path installDirDS2 = path / "common/Dungeon Siege 2";

            if (fs::is_directory(installDirDS2))
            {
                config.setString("ds2-install-path", installDirDS2.string());
            }
        }
    }
}
